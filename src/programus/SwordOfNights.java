...
public class SwordOfNights {
    ...
    public int solution(int number, int limit, int power) {
        ...
        for (int i = 1; i <= number; i++) {
            int attackPowers = countDivisors(i);
            if (attackPowers > limit) {
                attackPowers = power;
            }
            sum += attackPowers;
    ...
    public int solution3(int number, int limit, int power) {
        // 1부터 number까지의 약수 개수를 저장할 배열
        int[] countArray = new int[number + 1];

        // 1. 각 숫자의 약수 개수를 미리 계산 (체 방식)
        for (int i = 1; i <= number; i++) {
    ...
    public static void main(String[] args) {
        ...
        SwordOfNights swordOfNights = new SwordOfNights();
        System.out.println(swordOfNights.solution(5, 3, 2)); //10
        System.out.println(swordOfNights.solution(10, 3, 2)); //21
